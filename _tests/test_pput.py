...
class ReadOnlyFile(object):
    ...
    def __getattr__(self, name):
        ...
        raise AssertionError(
            "this file-like-object is readonly, {} is now allowed".format(name)
        )
    ...
    if _cached_sample_data is None:
        ...
        for count in range(6):
            ...
            for _ in range(2 * 1024):
                ...
                data.write("".join(cc + chars[i] for i in range(256)))
...
def test_multipart_etag(sample_data):
    ...
    while True:
        ...
        if len(chunk) == 0:
            break
        digest = hashlib.md5()
        digest.update(chunk.encode("latin1"))
...
class DummyWorker(UploadWorker):
    def upload_part(self, index, chunk):
        return hashlib.md5(chunk.encode("latin1")).hexdigest()
