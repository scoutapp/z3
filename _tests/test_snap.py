...
class FakeBucket(object):
    rand_prefix = (
        "test-" + "".join([random.choice(string.ascii_letters) for _ in range(8)]) + "/"
    )
    fake_data = {
    ...
    def list(self, *a, **kwa):
        ...
        return (
            FakeKey(os.path.join(self.rand_prefix, name))
            for name in self.fake_data.keys()
        )

    def get_key(self, key):
        name = key[len(self.rand_prefix) :]
        return FakeKey(name=key, metadata=self.fake_data[name])
...
def write_s3_data():
    ...
    """
    cfg = get_config()
    bucket = boto.connect_s3(cfg["S3_KEY_ID"], cfg["S3_SECRET"]).get_bucket(
        cfg["BUCKET"]
    )
...
def s3_manager(request):
    ...
    return S3SnapshotManager(
        request.param(),
        s3_prefix=FakeBucket.rand_prefix,
...
def test_list_local_snapshots():
    ...
    expected = {
        "pool": OrderedDict(
            [  # _parse_snapshots returns snapshots for ALL filesystems
                (
                    "snap_p1",
                    {
                        "name": "pool@snap_p1",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("pool", [("pool@snap_p1", None), ("pool@snap_p2", "pool@snap_p1")]),
    ],
)
def test_zfs_list(fs_name, expected):
    zfs = FakeZFSManager(fs_name=fs_name, snapshot_prefix="snap_")
    actual = [
        (snap.name, snap.parent.name if snap.parent else None) for snap in zfs.list()
    ]
...
class FakeCommandExecutor(CommandExecutor):
    ...
    def shell(
        self, cmd, dry_run=None, capture=None
...
def test_pair_list(pair_manager):
    ...
    names = [(name(s3_snap), name(z_snap)) for (s3_snap, z_snap) in pairs]
    expected = [
...
def test_backup_latest_full(pair_manager):
    ...
    assert pair_manager._cmd._called_commands == [
        e.format(FakeBucket.rand_prefix) for e in expected
    ]
...
def test_backup_incremental_missing_parent(s3_manager):
    ...
    zfs_manager = FakeZFSManager(
        fs_name="pool/fs", expected=expected, snapshot_prefix="snap_"
    )
    fake_cmd = FakeCommandExecutor()
    pair_manager = PairManager(s3_manager, zfs_manager, command_executor=fake_cmd)
    with pytest.raises(IntegrityError) as excp_info:
        pair_manager.backup_incremental()
    assert "Broken snapshot detected pool/fs@snap_5, reason: 'parent broken'" in str(
        excp_info
    )
...
def test_backup_incremental_cycle(s3_manager):
    ...
    zfs_manager = FakeZFSManager(
        fs_name="pool/fs", expected=zfs_list, snapshot_prefix="snap_"
    )
    fake_cmd = FakeCommandExecutor()
    pair_manager = PairManager(s3_manager, zfs_manager, command_executor=fake_cmd)
    with pytest.raises(IntegrityError) as excp_info:
...
def test_restore_force(s3_manager):
    ...
    fake_cmd = FakeCommandExecutor()
    pair_manager = PairManager(s3_manager, zfs_manager, command_executor=fake_cmd)
    pair_manager.restore(
        "pool/fs@snap_3", force=True
    )  # ask for an incremental snapshot
    # all incremental snapshots until we hit a full snapshot are expected
    expected = [
...
def test_get_latest():
    ...
    assert (
        ...
        'SNAPSHOT_PREFIX="zfs-auto-snap:daily" is correct?' in str(excp_info)
    )
    assert fake_cmd._called_commands == []
